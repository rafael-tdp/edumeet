...
//		GroupBy(friendship.FieldStatus).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (fq *FriendshipQuery) loadFriend(ctx context.Context, query *UserQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *User)) error {
	...
	nodeids := make(map[string][]*Friendship)
	for i := range nodes {
		if nodes[i].user_friendships == nil {
			continue
		}
		fk := *nodes[i].user_friendships
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_friendships" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
