...
func (dq *DocumentQuery) QueryEventDocuments() *EventDocumentQuery {
	query := (&EventDocumentClient{config: dq.config}).Query()
...
func (dq *DocumentQuery) QueryMessage() *MessageQuery {
...
func (dq *DocumentQuery) WithEventDocuments(opts ...func(*EventDocumentQuery)) *DocumentQuery {
	query := (&EventDocumentClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withEventDocuments = query
	return dq
}
...
func (dq *DocumentQuery) WithMessage(opts ...func(*MessageQuery)) *DocumentQuery {
	query := (&MessageClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := dq.withEventDocuments; query != nil {
		if err := dq.loadEventDocuments(ctx, query, nodes,
			func(n *Document) { n.Edges.EventDocuments = []*EventDocument{} },
			func(n *Document, e *EventDocument) { n.Edges.EventDocuments = append(n.Edges.EventDocuments, e) }); err != nil {
			return nil, err
		}
	}
...
func (dq *DocumentQuery) loadEventDocuments(ctx context.Context, query *EventDocumentQuery, nodes []*Document, init func(*Document), assign func(*Document, *EventDocument)) error {
	...
	query.Where(predicate.EventDocument(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(document.EventDocumentsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.document_event_documents
		if fk == nil {
			return fmt.Errorf(`foreign-key "document_event_documents" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "document_event_documents" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (dq *DocumentQuery) loadMessage(ctx context.Context, query *MessageQuery, nodes []*Document, init func(*Document), assign func(*Document, *Message)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(document.MessageTable)
		s.Join(joinT).On(s.C(message.FieldID), joinT.C(document.MessagePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(document.MessagePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(document.MessagePrimaryKey[1]))
