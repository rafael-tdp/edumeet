...
func (*Friendship) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case friendship.ForeignKeys[0]: // friendship_user
			values[i] = new(sql.NullString)
		case friendship.ForeignKeys[1]: // user_friendships
			values[i] = new(sql.NullString)
		default:
...
func (f *Friendship) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case friendship.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field friendship_user", values[i])
			} else if value.Valid {
				f.friendship_user = new(string)
				*f.friendship_user = value.String
			}
