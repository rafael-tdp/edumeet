...
func (sq *SubjectQuery) QueryUsers() *UserQuery {
...
func (sq *SubjectQuery) QueryEvents() *EventQuery {
...
func (sq *SubjectQuery) WithUsers(opts ...func(*UserQuery)) *SubjectQuery {
	query := (&UserClient{config: sq.config}).Query()
...
func (sq *SubjectQuery) WithEvents(opts ...func(*EventQuery)) *SubjectQuery {
	query := (&EventClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subject, error) {
	...
	if query := sq.withUsers; query != nil {
		if err := sq.loadUsers(ctx, query, nodes,
			func(n *Subject) { n.Edges.Users = []*User{} },
			func(n *Subject, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	if query := sq.withEvents; query != nil {
		if err := sq.loadEvents(ctx, query, nodes,
			func(n *Subject) { n.Edges.Events = []*Event{} },
			func(n *Subject, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubjectQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Subject)
	nids := make(map[string]map[*Subject]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(subject.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(subject.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(subject.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(subject.UsersPrimaryKey[1]))
...
func (sq *SubjectQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *Event)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Subject)
	nids := make(map[string]map[*Subject]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(subject.EventsTable)
		s.Join(joinT).On(s.C(event.FieldID), joinT.C(subject.EventsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(subject.EventsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(subject.EventsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
