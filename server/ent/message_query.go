...
func (mq *MessageQuery) QueryEvent() *EventQuery {
...
func (mq *MessageQuery) QueryFriendship() *FriendshipQuery {
	query := (&FriendshipClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MessageQuery) QueryDocuments() *DocumentQuery {
...
func (mq *MessageQuery) Clone() *MessageQuery {
	...
	return &MessageQuery{
		...
		withFriendship: mq.withFriendship.Clone(),
...
func (mq *MessageQuery) WithEvent(opts ...func(*EventQuery)) *MessageQuery {
	query := (&EventClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) WithFriendship(opts ...func(*FriendshipQuery)) *MessageQuery {
	query := (&FriendshipClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withFriendship = query
	return mq
}
...
func (mq *MessageQuery) WithDocuments(opts ...func(*DocumentQuery)) *MessageQuery {
	query := (&DocumentClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	var (
		...
		loadedTypes = [4]bool{
			mq.withUser != nil,
			mq.withEvent != nil,
			mq.withFriendship != nil,
			mq.withDocuments != nil,
		}
	)
	if mq.withUser != nil || mq.withEvent != nil || mq.withFriendship != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := mq.withEvent; query != nil {
		if err := mq.loadEvent(ctx, query, nodes, nil,
			func(n *Message, e *Event) { n.Edges.Event = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withFriendship; query != nil {
		if err := mq.loadFriendship(ctx, query, nodes, nil,
			func(n *Message, e *Friendship) { n.Edges.Friendship = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withDocuments; query != nil {
		if err := mq.loadDocuments(ctx, query, nodes,
			func(n *Message) { n.Edges.Documents = []*Document{} },
			func(n *Message, e *Document) { n.Edges.Documents = append(n.Edges.Documents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MessageQuery) loadEvent(ctx context.Context, query *EventQuery, nodes []*Message, init func(*Message), assign func(*Message, *Event)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Message)
	for i := range nodes {
		if nodes[i].event_messages == nil {
			continue
		}
		fk := *nodes[i].event_messages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "event_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadFriendship(ctx context.Context, query *FriendshipQuery, nodes []*Message, init func(*Message), assign func(*Message, *Friendship)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Message)
	for i := range nodes {
		if nodes[i].friendship_messages == nil {
			continue
		}
		fk := *nodes[i].friendship_messages
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "friendship_messages" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MessageQuery) loadDocuments(ctx context.Context, query *DocumentQuery, nodes []*Message, init func(*Message), assign func(*Message, *Document)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Message)
	nids := make(map[string]map[*Message]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(message.DocumentsTable)
		s.Join(joinT).On(s.C(document.FieldID), joinT.C(message.DocumentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(message.DocumentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(message.DocumentsPrimaryKey[0]))
